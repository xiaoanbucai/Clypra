export interface RetroComicConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class RetroComicEngine {
  private cfg: Required<RetroComicConfig>;

  constructor(config: RetroComicConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<RetroComicConfig> = {
      width: 800,
      height: 200,
      text: "BOOM!",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "RetroComic".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
