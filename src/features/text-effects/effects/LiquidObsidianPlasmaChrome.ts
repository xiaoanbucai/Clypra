export interface LiquidObsidianPlasmaChromeConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class LiquidObsidianPlasmaChromeEngine {
  private cfg: Required<LiquidObsidianPlasmaChromeConfig>;

  constructor(config: LiquidObsidianPlasmaChromeConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<LiquidObsidianPlasmaChromeConfig> = {
      width: 800,
      height: 200,
      text: "Classic Ink",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "LiquidObsidianPlasmaChrome".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
