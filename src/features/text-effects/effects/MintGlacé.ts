export interface MintGlacéConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class MintGlacéEngine {
  private cfg: Required<MintGlacéConfig>;

  constructor(config: MintGlacéConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<MintGlacéConfig> = {
      width: 800,
      height: 200,
      text: "FROSTED",
      fontFamily: "Raleway",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "MintGlacé".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
...
export const MintGlacéDefinition = {
  id: "mint-glacé",
  name: "Mint Glacé",
  text: "CLYPRA",
  category: "classic",
