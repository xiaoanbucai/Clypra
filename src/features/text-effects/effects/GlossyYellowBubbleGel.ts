export interface GlossyYellowBubbleGelConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class GlossyYellowBubbleGelEngine {
  private cfg: Required<GlossyYellowBubbleGelConfig>;

  constructor(config: GlossyYellowBubbleGelConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<GlossyYellowBubbleGelConfig> = {
      width: 800,
      height: 200,
      text: "Classic Ink",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "GlossyYellowBubbleGel".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
